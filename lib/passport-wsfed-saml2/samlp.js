...
function getResponseExpiration(samlResponse, options) {
  var conditions = xpath.select(".//*[local-name(.)='Conditions']", samlResponse);
  var clockSkew = options.clockSkew || 3; // we need to implement options.clockSkew - default to 3 minutes

  if (!conditions || conditions.length === 0) {
    return new Date().setMinutes(15 + clockSkew); // this should be a default Max Age that we determine
  }

  var notOnOrAfter = new Date(conditions[0].getAttribute('NotOnOrAfter'));
  notOnOrAfter = notOnOrAfter.setMinutes(notOnOrAfter.getMinutes() + clockSkew);
...
Samlp.prototype = {
  ...
  validateSamlResponse: function (samlResponse, callback) {
    ...
    var responseExpirationDate = getResponseExpiration(samlResponse, self.options);

    self.isValidResponseID(responseXML.getAttribute('ID'), responseExpirationDate, function(err){
